...
| `claude.png` | Same bitmap as `desktop/src/features/onboarding/assets/harness-logos/claude.png` (RuntimeIcon inline fallback) | Copied 2026-09-01 | Nominative use to identify the Claude Code harness | Onboarding asset | Public copy so chat/mention `<img>` surfaces can load it |
| `goose.svg` | [block/goose](https://github.com/block/goose) path already in `HarnessMarks.tsx` | Same as inline Goose mark | Apache-2.0 © Block, Inc. | `documentation/static/img/goose.svg` | Badge circle + light fill so the mark works as an `<img>` / mention-chip background |
| `cursor.svg` | [simple-icons](https://github.com/simple-icons/simple-icons) path already in `HarnessMarks.tsx` | `16.27.1` (slug `cursor`) | CC0-1.0 (path data); nominative use of the Cursor mark | `icons/cursor.svg` | Badge circle + light fill so the mark works as an `<img>` / mention-chip background |
| `terminal.svg` | Original Crew mark | 2026-09-01 | Original | — | Neutral terminal glyph for Codex (no vendor blossom; same fallback RuntimeIcon uses) |
