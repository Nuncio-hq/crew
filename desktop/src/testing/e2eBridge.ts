...
function emitMockObserverControlResult(
  ...
  const payload = {
    ...
    turnId: result.turnId ?? request.turnId,
...
async function handleGetChannelReconnectRepair(
  ...
  const kinds = new Set([
    5, 7, 9, 9005, 40001, 40002, 40003, 40008, 40099, 45001, 45003, 46043,
...
async function submitSignedEvent(
  ...
  const result = await relayJsonRequest<{
    event_id: string;
    accepted: boolean;
    message: string;
  }>(config, "/events", {
  ...
  return { ...result, created_at: signed.created_at };
}
...
async function handleListRelayAgents(
  ...
  syncMockRelayAgentsFromManagedAgents();
  const viewer = getMockMemberPubkey(config).toLowerCase();
  return mockRelayAgents
    .filter(
      (agent) =>
        !config?.mock?.ownerOnlyAccessBuild ||
        agent.owner_pubkey?.toLowerCase() === viewer,
    )
    .map(cloneRelayAgent);
}
...
async function handleEditMessage(
  ...
  const extraTags = [
    ...mediaTags,
    ...emojiTags,
    ...mentionPubkeys.map((pubkey) => ["p", pubkey.toLowerCase()]),
    ...removedTags,
    ...(mentionTags ?? []),
