...
function entityLinkPresentation(link: ParsedEntityLink) {
  switch (link.type) {
    ...
    case "file":
      // Wiki `buzz://file` citations stay ordinary inline links (#289
      // unresolved vs the compact metadata renderer — do not chip).
      return {
        ariaLabel: `Open ${link.path} in repository ${link.dtag}`,
        icon: "repo" as const,
        label: `${link.dtag}:${link.path}`,
        tooltipFooter: "File",
        asChip: false,
      };
    default: {
...
/**
 * Markdown path: wraps `renderEntityLinkAnchor` so issue/PR tooltips can
 * read the authoritative Projects list.
 */
