...
import { dispatchBufferedLiveEvents } from "./relayLiveEventBuffer.ts";
...
test("buffer flush drops stale generations and removed subscriptions", () => {
  ...
  const item = (subId, sub, value, generation) => ({
    subId,
    subscription: sub,
    event: value,
  ...
  dispatchBufferedLiveEvents(
    [
      item("live-1", subscription, event("stale", 100), 6),
      item("removed", removedSubscription, event("removed", 101), 7),
      item("live-1", subscription, event("current", 102), 7),
    ],
