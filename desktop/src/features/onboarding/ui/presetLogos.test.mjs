...
const presetHarnessesRs = readFileSync(
  path.join(
    desktopRoot,
    "src-tauri/src/managed_agents/discovery/preset_harnesses.rs",
  ),
  "utf8",
);

const presetBlock = presetHarnessesRs.match(
  /const PRESET_HARNESSES: &\[PresetHarness\] = &\[([\s\S]*?)\n\];/,
);
assert.ok(
  presetBlock,
  "could not locate PRESET_HARNESSES in preset_harnesses.rs",
);
