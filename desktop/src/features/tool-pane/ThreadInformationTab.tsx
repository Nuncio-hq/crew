...
import { ThreadActivityRunControls } from "./ThreadActivityRunControls";

const EMPTY_MESSAGES: TimelineMessage[] = [];

// Keep the existing Activity/Plans first paint independent of the optional
// recap feature. Context loads the recap surface only when selected.
    ...
    >
      ...
      {context.rootEventId ? (
        <ThreadActivityRunControls
          key={`${channelId}:${context.rootEventId}:${conversationId}:${agent.agentPubkey}`}
          channelId={channelId}
