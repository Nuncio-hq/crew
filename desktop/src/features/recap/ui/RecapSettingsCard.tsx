...
export function RecapSettingsCard({
  ...
  const settingsError = snapshot.settingsError ?? null;
  ...
  function updateRuntime(runtimeId: string) {
    ...
    setDraft((current) => ({
      ...
      mode: "manual",
      runtimeId,
      // The backend only admits the single certified model; prefill it when
      // the inventory offers exactly one so save is not rejected outright.
      requestedModel:
        current.runtimeId === runtimeId && current.requestedModel
          ? current.requestedModel
          : nextRuntime?.models.length === 1
            ? nextRuntime.models[0]
            : null,
  ...
  return (
    <section className="min-w-0" data-testid="settings-recap">
      ...
      <SettingsOptionGroupList>
        {settingsError ? (
          <SettingsOptionGroup title="Saved settings">
            <SettingsOptionRow>
              <div className="min-w-0 flex-1 space-y-1">
                <p
                  className="text-sm font-medium"
                  data-testid="recap-settings-repair"
                >
                  Saved recap settings could not be applied
                </p>
                <p
                  className="text-xs text-muted-foreground/70"
                  data-settings-subcopy
                >
                  {recapErrorMessage(settingsError)} Save a fresh selection to
                  repair; nothing was changed automatically.
                </p>
              </div>
            </SettingsOptionRow>
          </SettingsOptionGroup>
            ...
            <SettingsOptionRow className="items-start">
              <div className="min-w-0 flex-1 space-y-1.5">
                ...
                <select
                  ...
                  onChange={(event) => {
                    const profileRef = event.target.value || null;
                    setDraft((current) => ({
                      ...current,
                      profileRef,
                      // A certified profile supplies its model; the backend
                      // still requires requestedModel to name it.
                      requestedModel: profileRef
                        ? (runtime.models[0] ?? current.requestedModel)
                        : current.requestedModel,
                    }));
                    setStatus(null);
                  }}
