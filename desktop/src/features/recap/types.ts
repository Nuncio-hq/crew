...
export type RecapSettingsSnapshot = {
  ...
  /** Recoverable owner-local settings read/validation error; settings render as Off. */
...
export function hasValidRecapSelection(
  ...
  // Mirror validate_settings in managed_agents::recap_commands: a supported
  // runtime admits exactly one certified model, and manual mode must request
  // it — for hermes the selected profile supplies that model.
  ...
  if (settings.capabilityFingerprint !== runtime.capabilityFingerprint) {
    return false;
  }
