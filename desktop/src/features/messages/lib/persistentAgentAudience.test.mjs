...
test("explicit thread identity is shared across composer surfaces and isolates other roots", async () => {
  const store = await loadStore();
  const base = {
    ownerPubkey: ownerA,
    channelId: "channel-a",
  ...
  const thread = store.getPersistentAgentAudienceScope({
    ...base,
    composerKey: "thread:root-a",
  });
  const workbench = store.getPersistentAgentAudienceScope({
    ...base,
    composerKey: "workbench:root-a",
  });
  const other = store.getPersistentAgentAudienceScope({
    ...base,
    threadRootId: "root-b",
    composerKey: "workbench:root-a",
  });
  assert.equal(thread, `${ownerA}:channel-a:thread:root-a`);
  assert.equal(workbench, thread);
  assert.notEqual(other, thread);
});

test("unresolved explicit thread roots cannot inherit a draft or channel audience", async () => {
  const store = await loadStore();
  for (const threadRootId of [null, "", "  "]) {
    assert.equal(
      store.getPersistentAgentAudienceScope({
        ownerPubkey: ownerA,
        channelId: "channel-a",
        composerKey: "thread:previous-root",
        threadRootId,
      }),
      null,
    );
