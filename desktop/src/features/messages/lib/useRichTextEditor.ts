...
export type RichTextEditorOptions = {
  ...
  /** Lowercased agent display name → avatar URL for composer mention chips. */
  ...
  const editor = useEditor(
    {
      ...
      editorProps: {
        ...
        attributes: {
          autocapitalize: "none",
          autocorrect: "off",
          class: `${MESSAGE_MARKDOWN_CLASS} min-h-0 resize-none overflow-y-hidden border-0 bg-transparent px-0 py-0 text-message text-foreground shadow-none focus-visible:ring-0 caret-foreground outline-hidden max-w-none`,
