...
test("migrates an existing explicit preference without importing saved recipients", async () => {
  values.clear();
  persistentAudience.resetPersistentAgentAudienceStore();
  values.set("buzz:keep-addressed-agents-active", "1");
  values.set(
    "buzz:persistent-agent-audiences:v2",
    JSON.stringify({ old: ["a".repeat(64)] }),
  );
  const migrated = await import(
    "./autoPinMentionedAgentsPreference.ts?migration=enabled"
  );
  assert.equal(migrated.getKeepMentionedAgentsPinned(), true);
  assert.equal(
    values.get(migrated.KEEP_MENTIONED_AGENTS_PINNED_STORAGE_KEY),
    "true",
  );
  assert.deepEqual(
    persistentAudience.getPersistentAgentAudienceSnapshot().audiences,
...
test("new preference wins over a retained older preference on subsequent loads", async () => {
  values.set("buzz:keep-addressed-agents-active", "1");
  values.set(preference.KEEP_MENTIONED_AGENTS_PINNED_STORAGE_KEY, "false");
  const reloaded = await import(
    "./autoPinMentionedAgentsPreference.ts?migration=new-wins"
  );
  assert.equal(reloaded.getKeepMentionedAgentsPinned(), false);
});
