...
export function resolveComposerMessageLinkAttributes(
  ...
  switch (entity.value.type) {
    ...
    case "file":
      // Wiki file citations stay ordinary composer links, not chips.
      return null;
    default: {
      const _exhaustive: never = entity.value;
      return _exhaustive;
    }
  }
}
...
function composerLinkPresentation(
  ...
  switch (entity.value.type) {
    case "repo":
      return {
        ariaLabel: `Open repository ${entity.value.dtag}`,
        channelName: "",
        dataAttributes: { "data-buzz-link-kind": "repo" },
        icon: "repo",
        label: entity.value.dtag,
      };
    case "project":
      return {
        ariaLabel: `Open project ${entity.value.dtag}`,
        channelName: "",
        dataAttributes: { "data-buzz-link-kind": "project" },
        icon: "project",
        label: entity.value.dtag,
      };
    case "pr":
      return {
        ariaLabel: `Open pull request ${entity.value.id.slice(0, 8)} in repository ${entity.value.dtag}`,
        channelName: "",
        dataAttributes: { "data-buzz-link-kind": "pr" },
        icon: "pr",
        label: entity.value.dtag,
      };
    case "issue":
      return {
        ariaLabel: `Open issue ${entity.value.id.slice(0, 8)} in repository ${entity.value.dtag}`,
        channelName: "",
        dataAttributes: { "data-buzz-link-kind": "issue" },
        icon: "issue",
        label: entity.value.dtag,
      };
    case "file":
      return {
        ariaLabel: `Open ${entity.value.path} in repository ${entity.value.dtag}`,
        channelName: "",
        dataAttributes: {},
        icon: "repo",
        label: entity.value.path,
      };
    default: {
      const _exhaustive: never = entity.value;
      return _exhaustive;
