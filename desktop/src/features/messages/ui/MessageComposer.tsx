...
function MessageComposerImpl({
  ...
  const audienceScope =
    audienceContext && channelId && ownerPubkey
      ? getPersistentAgentAudienceScope({
          ...
          threadRootId: audienceContext.threadRootId,
        })
      : null;
  ...
  const submitMessage = React.useCallback(async () => {
    ...
    const linkPreviewTags = getReadyLinkPreviewTags();
    const suppressLinkPreviews = linkPreviewTags.some(
      (tag) => tag[0] === "link-preview" && tag[1] === "none",
    );
    // Edit mode
    ...
    try {
      const preparedLinkPreviews = suppressLinkPreviews
        ? null
      ...
      await mentionSendFlow.sendMessageWithMentionFlow({
        ...
        linkPreviewTags: preparedLinkPreviews ? [] : linkPreviewTags,
    ...
    <>
      ...
      >
        ...
        <div className="relative flex w-full flex-col gap-0">
          ...
          <form
            className={cn(
              "relative z-10 isolate @container rounded-2xl border border-border/50 bg-background/80 px-3 pb-2 pt-3 shadow-none supports-[backdrop-filter]:bg-background/70 dark:bg-background/70 dark:supports-[backdrop-filter]:bg-background/55 [@container(min-width:40rem)]:px-4",
              layoutMode === "standalone" &&
