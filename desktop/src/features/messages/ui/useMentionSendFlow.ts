...
// Crew (D-022): workspace / visible-page context + Project-thread routing.
import { applyCrewSendContext, useCrewSendContext } from "./crewSendContext";
...
export function useMentionSendFlow({
  ...
  const crewSend = useCrewSendContext();
      ...
      try {
        ...
        const finishSend = async (
          ...
          if (signal?.aborted || isSendCancelled())
            return restoreComposerAfterFailure();
          // Crew: workspace + visible-page context, Project-thread routing.
          const crew = await applyCrewSendContext(crewSend, {
            content: finalContent,
            channelId: sendChannelId ?? draft.capturedChannelId ?? "",
            draft,
            revalidatedMentionPubkeys,
            reportError: setNonMemberPromptError,
          });
          if (signal?.aborted || isSendCancelled())
            return restoreComposerAfterFailure();
          ...
          const notified = new Set(crew.mentionPubkeys.map(normalizePubkey));
