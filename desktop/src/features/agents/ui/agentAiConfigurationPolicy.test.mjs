...
test("a profile-owned model never asks for a hidden model selection", () => {
  // Crew: Hermes write-through hides the model field entirely
  // (needsModelSelection=false), so a satisfied pair must not be explained.
  assert.equal(
    agentAiConfigurationSubmitBlockReason(
      "custom",
