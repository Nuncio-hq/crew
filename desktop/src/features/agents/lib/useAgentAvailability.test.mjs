...
for (const presence of ["online", "away", "offline", undefined]) {
  test(`retained deployment receipt does not supply availability (${presence})`, () => {
    ...
    assert.equal(html.includes("bg-success"), availability === "online");
...
for (const [loaded, connected] of [
  ...
  test(`unavailable presence is unknown, not cached online (${loaded}, ${connected})`, () => {
    ...
    assert.doesNotMatch(html, /bg-success|is running/);
  });
}
...
for (const availability of ["online", "away"]) {
  test(`runtime error remains actionable while other thread workers are ${availability}`, () => {
    const html = renderToStaticMarkup(
      createElement(AgentRuntimeAvatarControl, {
        activeTestId: "active",
        startTestId: "start",
    ...
    assert.match(html, /data-testid="error"/);
    assert.doesNotMatch(html, /data-testid="active"/);
  });
}
