...
export function createOtherSetupAgentChecker({
  currentPubkey,
  managedAgentsQuery,
  relayAgentsQuery,
}: {
  ...
  managedAgentsQuery: {
    data?: readonly ManagedAgent[];
    error: unknown;
    isFetching: boolean;
  };
  relayAgentsQuery: {
    data?: readonly RelayAgent[];
    error: unknown;
    isFetching: boolean;
  };
}) {
  ...
  return (profileOwnerPubkey: string | null | undefined, pubkey: string) =>
    isOtherSetupAgent({
