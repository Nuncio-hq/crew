...
pub(crate) use metadata::{
    apply_agent_display_env, apply_replay_floor_env, apply_system_prompt_env,
    child_rust_log_filter, resolve_session_title, runtime_metadata_env_vars, DISPLAY_NAME_ENV_VAR,
    REPLAY_FLOOR_ENV_VAR, SESSION_TITLE_ENV_VAR,
};
...
pub fn build_managed_agent_summary(
    ...
    Ok(ManagedAgentSummary {
        ...
        agent_command: descriptor.command.clone(),
        ...
        profile_readiness: crate::managed_agents::hermes_profile_readiness(
            &descriptor.command,
            record.hermes_profile.as_deref(),
        ),
...
#[path = "runtime/cli-environment.rs"]
mod cli_environment;
pub(crate) use cli_environment::configure_runtime_cli;
...
pub fn spawn_agent_child(
    ...
    // Shared compute maps `auto` → MeshLLM `mesh`. Cursor skips `BUZZ_ACP_MODEL`
    // (model is pinned on argv via `inject_cursor_startup_model_arg`).
    #[cfg(feature = "mesh-llm")]
    let acp_model = super::resolve_buzz_acp_model_env(
        effective_command,
        &mesh_model_id,
    ...
    let acp_model =
        super::resolve_buzz_acp_model_env(effective_command, effective_model.as_deref());
    ...
    command.env_remove(metadata::REPLAY_FLOOR_ENV_VAR);
    ...
    let spawn_started_at_ms = super::transport_status::configure_child_transport_stamp(&mut command, transport_storage_available, &start_nonce, owner_hex, spawned_setup_mode)?;
...
pub fn start_managed_agent_process(
    ...
    #[cfg(not(windows))]
    {
        start_managed_agent_process_supported(
            app,
            record,
            runtimes,
            owner_hex,
