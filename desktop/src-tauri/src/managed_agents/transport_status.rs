...
mod tests {
    ...
    fn transport_environment_keys_are_reserved_from_user_overrides() {
        ...
        assert!(super::super::env_vars::is_reserved_env_key(STATUS_VERSION_ENV));
        assert!(super::super::env_vars::is_reserved_env_key(SPAWN_STARTED_AT_ENV));
    ...
    fn missing_owner_and_adopted_generation_remain_unmonitored() {
        ...
        let nonce = uuid::Uuid::new_v4().simple().to_string();
        assert!(
            registered_monitor(&key, log, &nonce, 1, 1, None, false, &diagnostics).is_none()
        );
        assert!(
            registered_monitor(
                &key,
    ...
    fn registration_binds_native_owner_and_clears_retired_token() {
        ...
        assert!(diagnostics
            ...
            .is_some());
        let monitor = registered_monitor(&key, &log, &nonce, 1, 1, Some(&owner), false, &diagnostics)
