...
mod replay_floor_tests {
    ...
    #[test]
    fn caller_floor_follows_user_layer_while_floorless_start_does_not_inherit_parent() {
        let mut command = std::process::Command::new("unused");
        command.env_remove(REPLAY_FLOOR_ENV_VAR);
        apply_replay_floor_env(&mut command, None);
        assert!(command
            .get_envs()
            .any(|(key, value)| key == REPLAY_FLOOR_ENV_VAR && value.is_none()));
        command.env(REPLAY_FLOOR_ENV_VAR, "9999");
        apply_replay_floor_env(&mut command, Some(1000));
        assert!(command
            .get_envs()
            .any(|(key, value)| key == REPLAY_FLOOR_ENV_VAR
                && value == Some(std::ffi::OsStr::new("1000"))));
    }

    #[cfg(unix)]
    #[test]
    fn real_child_receives_the_caller_replay_floor() {
        let mut command = std::process::Command::new("/bin/sh");
        command.args(["-c", "printf '%s' \"$BUZZ_ACP_REPLAY_FLOOR\""]);
        command.env(REPLAY_FLOOR_ENV_VAR, "9999");
        apply_replay_floor_env(&mut command, Some(1000));
