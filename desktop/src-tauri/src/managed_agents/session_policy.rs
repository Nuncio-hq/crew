...
impl Default for ManagedAgentExperimentState {
    fn default() -> Self {
        Self {
            ...
            thread_scoped_acp_sessions_enabled: AtomicBool::new(true),
        }
    }
}
...
pub(crate) fn acp_session_policy(_state: &AppState) -> AcpSessionPolicy {
    // Crew's session ledger always isolates workers by thread, regardless of
    // older renderer experiment values received during a workspace switch.
...
#[cfg(test)]
mod crew_policy_tests {
    #[test]
    fn stale_renderer_channel_experiment_cannot_change_thread_worker_ledger() {
        let state = crate::app_state::build_app_state();
        state
            .thread_scoped_acp_sessions_enabled()
            .store(false, std::sync::atomic::Ordering::Release);
        assert_eq!(
            super::acp_session_policy(&state),
            super::AcpSessionPolicy::Thread
        );
    }
}
