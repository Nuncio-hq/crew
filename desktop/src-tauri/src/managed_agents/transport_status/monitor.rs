...
use buzz_core_pkg::transport_status::{
    TransportConnectionAttempt, TransportLease, TransportRecordEnvelope, TransportRecordV2,
    TransportReceivedAuth, TransportStatus,
};

use super::export::{
    auth_evidence_from_record, AuthCandidate, ExportCandidate, ExportState,
};
...
impl Monitor {
    ...
    pub fn enable(&mut self, owner: &str) -> Result<bool, String> {
        ...
        if changed {
            ...
            self.lease.unavailable();
            self.export
                .rebind(self.ticket.clone(), current_wall_ms().max(1), Instant::now());
        }
        Ok(changed)
    }
