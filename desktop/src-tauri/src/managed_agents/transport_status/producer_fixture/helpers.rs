...
use super::{
    ...
    CAPTURED_BINARY_BYTES, CAPTURED_SOURCE_BYTES, CAPTURED_SOURCE_FILES,
    CAPTURED_STREAM_BYTES, CAPTURE_DRAIN_DEADLINE, POLL_INTERVAL,
};
...
fn source_tree_sha256(root: &Path, deadline: Instant) -> Result<String, String> {
    ...
    for relative in files {
        ...
        if !metadata.is_file() {
            return Err(format!("source binding entry is no longer a regular file: {}", path.display()));
        }
        hasher.update(relative.to_string_lossy().as_bytes());
        hasher.update([0]);
        let mut file = File::open(&path).map_err(|error| format!("open source {}: {error}", path.display()))?;
        let mut bytes = [0_u8; 64 * 1024];
        let mut total = 0_u64;
        loop {
            check_deadline(deadline, "source tree hash")?;
            let read = file.read(&mut bytes).map_err(|error| format!("read source {}: {error}", path.display()))?;
            if read == 0 {
                break;
            }
            total = total.saturating_add(read as u64);
            if total > CAPTURED_SOURCE_BYTES {
                return Err(format!("source binding file exceeds {} bytes: {}", CAPTURED_SOURCE_BYTES, path.display()));
            }
...
fn collect_source_files(
    ...
    for entry in entries {
        ...
        if metadata.file_type().is_symlink() {
            return Err(format!("source binding contains a symlink: {}", path.display()));
        }
        if metadata.is_dir() {
...
pub(super) fn create_private_dir(path: &Path) -> Result<(), String> {
    fs::create_dir_all(path).map_err(|error| format!("create private directory {}: {error}", path.display()))?;
    fs::set_permissions(path, fs::Permissions::from_mode(0o700))
...
impl FixtureRoot {
    pub(super) fn new() -> Result<Self, String> {
        ...
        let paths = ["bin", "config", "data", "cache", "home", "cwd", "logs", "tmp"]
...
impl ChildGuard {
    ...
    pub(super) async fn wait_until_exit(&mut self, deadline: Instant) -> Result<ExitStatus, String> {
        loop {
            let child = self
                .child
                .as_mut()
...
async fn finish_capture(
    ...
    match tokio::time::timeout(timeout, &mut task).await {
        ...
        Err(_) => {
            task.abort();
            let _ = task.await;
            Err(format!("ACP fixture {stream} capture exceeded its deadline"))
        }
    }
}
