...
async fn run_whole_chain_fixture() -> Result<(), String> {
    ...
    .ok_or_else(|| "production native monitor registration refused fixture generation".to_string())?;
    let ticket = monitor
        .snapshot()
    ...
    if raw.windows(DENIAL.len()).any(|window| window == DENIAL.as_bytes()) {
    ...
    let applied = diagnostics
        ...
        .apply(&ticket, final_envelope.clone(), false, Instant::now(), final_wall);
    if !applied {
    ...
    if !evidence.retired
        ...
        || evidence.record.received_auth.as_ref().map(|auth| &auth.auth_event_id)
            != Some(&auth_event_id)
    ...
    F: Fn(&TransportRecordEnvelope) -> bool,
{
    loop {
        if Instant::now() >= deadline {
            return Err(format!("waiting for status snapshot {} timed out", path.display()));
        }
...
impl ServerGuard {
    ...
    async fn finish(&mut self, deadline: Instant) -> Result<(), String> {
        ...
        match tokio::time::timeout(remaining, &mut handle).await {
            Ok(Ok(result)) => result
                .map_err(|error| format!("loopback server task failed: {error}")),
