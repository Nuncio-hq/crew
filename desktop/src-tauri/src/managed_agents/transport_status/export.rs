...
use buzz_core_pkg::transport_status::{
    TransportCode, TransportRecordV2, TransportState,
};
...
impl ExportState {
    ...
    pub(crate) fn rebind(&mut self, ticket: ReadTicket, native_registered_at_ms: u64, now: Instant) {
    ...
    pub(crate) fn observe_auth(&mut self, candidate: AuthCandidate, now: Instant) {
        ...
        if self
            .pending
            .as_ref()
            .is_some_and(|pending| matches!(pending, PendingMarker::Auth(key) if key != &candidate.key))
        {
            self.pending = None;
        }
    ...
    pub(crate) fn take_due(&mut self, now: Instant) -> Option<ExportCandidate> {
        ...
        if let Some(registration) = self.registration.as_mut().filter(|track| {
            ...
            return Some(ExportCandidate::Registration(registration.candidate.clone()));
        }
        if self.registration.as_ref().is_some_and(|track| !track.emitted) {
            return None;
        }
