...
/// aliased to the same value as `BUZZ_ACP_POOL_IDLE_TIMEOUT`). The next accepted
/// event re-wakes it through the same lazy path. Crew founder policy: 30 minutes
/// (1800s) so a warm pool survives a normal back-and-forth but a truly quiet
/// harness stops paying for workers.
const IDLE_POOL_SLEEP_SECS: &str = "1800";
