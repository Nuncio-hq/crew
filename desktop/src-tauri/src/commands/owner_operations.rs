...
/// adapter as the renderer command; native acceptance uses this sibling seam
/// to reopen the exact journal across app instances.
...
pub(crate) async fn replace_wiki_with_successor(
    ...
    replace_wiki_with_successor_at_path(
        app.clone(),
        journal_path(&app)?,
        expected,
        id,
...
/// Atomically retire one Wiki row and reserve its successor at a trusted
/// native journal path.
pub(super) async fn replace_wiki_with_successor_at_path<R: Runtime>(
    app: AppHandle<R>,
    path: PathBuf,
    expected: OwnerScopeToken,
    id: String,
