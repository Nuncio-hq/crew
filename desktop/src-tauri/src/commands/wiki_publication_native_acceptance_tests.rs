...
use super::wiki_publication_commands::current_row_at_path;
...
fn write_receipt(path: &Path, value: &Value) -> Result<(), String> {
    ...
    std::fs::set_permissions(path, std::fs::Permissions::from_mode(0o600))
        .map_err(|_| "native acceptance receipt permissions failed".into())
}
...
async fn native_wiki_publication_roundtrip_against_owned_relay() -> Result<(), String> {
    ...
    let (_, reserved_record) = current_row_at_path(
        app_a.handle().clone(),
        cfg.journal.clone(),
        &scope_a,
        &operation.id,
    ...
    .await?;
    let (row_before_restart, _) = current_row_at_path(
        app_a.handle().clone(),
        cfg.journal.clone(),
        &scope_a,
        &operation.id,
