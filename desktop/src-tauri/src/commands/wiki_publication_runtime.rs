...
impl<R: Runtime> NativeWikiPublication<R> {
    ...
    /// Construct a publication with an explicit journal and clock context.
    ///
    /// Production callers use the trusted app-data journal and system clock;
    /// native acceptance injects a disposable journal and deterministic clock
    /// while exercising the same captured identity and transport path.
