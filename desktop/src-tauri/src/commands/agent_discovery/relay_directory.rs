...
mod tests {
    ...
    fn marked_build_requires_verified_viewer_ownership() {
        ...
        let mut agents = vec![
            RelayAgentInfo {
                ...
                name: "Verified own agent".to_string(),
        ...
        let mut other_owner_agent = agents[0].clone();
        other_owner_agent.owner_pubkey = Some("d".repeat(64));
        agents.push(other_owner_agent);
        retain_agents_allowed_by_build(&mut agents, true, &cross_owner);

        assert_eq!(agents.len(), 1);
        assert_eq!(agents[0].name, "Verified own agent");
        assert_eq!(
    ...
    fn oss_build_preserves_ownerless_legacy_agents() {
        ...
        retain_agents_allowed_by_build(&mut agents, false, &"b".repeat(64));
