...
pub async fn search_messages(
    ...
    let cap = search_messages_limit(limit).min(100);
...
pub struct EditMessageInput {
    ...
    // Crew: pubkeys of mentions the edit *drops*. Emitted as `p-removed`
    // so the agent harness can undo a still-queued request.
    #[serde(default)]
    removed_mention_pubkeys: Vec<String>,
}
