//! Locking and scoped cache accessors for desktop application state.
use super::*;
...
impl AppState {
    /// Pending owner entries remain scoped to the active signing identity.
    pub(crate) fn pending_owned_channel_ids(&self, pubkey: &str) -> Vec<String> {
        self.pending_owned_channels
            .lock()
            .map(|pending| {
                pending
                    .iter()
                    .filter(|(owner, _)| owner == pubkey)
