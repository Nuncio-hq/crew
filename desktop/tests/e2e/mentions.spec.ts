...
test("duplicate owned agents preserve provenance and exact pubkey selection", async ({
  ...
  const remoteSidebarMarker = page.getByTestId(
    `sidebar-member-other-setup-${relayPubkey}`,
  );
  await expect(
    page.getByTestId(`sidebar-member-other-setup-${managedPubkey}`),
  ).toHaveCount(0);
  ...
  const localSidebarMarker = page.getByTestId(
    `sidebar-member-other-setup-${managedPubkey}`,
  );
  ...
  await page.getByTestId("members-sidebar").screenshot({
    path: "test-results/crew-release-evidence/agent-provenance-focus.png",
  });
});
...
test("owner-only builds hide cross-owner relay agents even when authorized for anyone", async ({
  page,
}) => {
  await installMockBridge(page, {
...
test("duplicate same-owner agents identify the other setup only after directories settle", async ({
  ...
  await expect(localRow.getByTestId("mention-agent-provenance")).toHaveCount(0);
  await expect(remoteRow.getByTestId("mention-agent-provenance")).toBeVisible();
});
