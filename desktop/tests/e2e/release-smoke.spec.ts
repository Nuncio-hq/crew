...
test("deep local-relay timeline remains bounded and every row is reachable", async ({
  ...
  const measurement = await measureAction(page, async () => {
    ...
    return {
      ...
      windowState: await page.evaluate((channelId) => {
        const client = window.__BUZZ_E2E_QUERY_CLIENT__ as unknown as
          | {
              getQueryData: (key: readonly string[]) =>
                | {
                    liveOverlay?: unknown[];
                    liveAux?: unknown[];
                    pages?: Array<{
          ...
          | undefined;
        const store = client?.getQueryData(["channel-window", channelId]);
        return {
          liveOverlayCount: store?.liveOverlay?.length ?? 0,
          liveAuxCount: store?.liveAux?.length ?? 0,
          pages: store?.pages?.map((entry) => ({
            rowCount: entry.rows.length,
            hasMore: entry.hasMore,
            nextCursor: entry.nextCursor,
          })),
          windowCalls: window.__BUZZ_E2E_COMMAND_LOG__?.filter(
