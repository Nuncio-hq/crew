...
/// Maximum queued events across all conversation UUIDs of a routing channel.
/// The same limit also bounds each individual conversation queue.
...
impl EventQueue {
    ...
    pub fn push(&mut self, event: QueuedEvent) -> bool {
        ...
        let conversation_id = event.channel_id;
        ...
        self.enforce_routing_channel_cap(conversation_id);
        true
    }
    ...
    pub fn requeue(&mut self, batch: FlushBatch) -> Option<FlushBatch> {
        ...
        self.enforce_routing_channel_cap(channel_id);
