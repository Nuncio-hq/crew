...
use buzz_core::transport_status::{
    TransportConnectionAttempt, TransportRecord, TransportRecordV2, TransportReceivedAuth,
    ...
    pub async fn write_snapshot(
        ...
        let bytes = if self.config.version == 2 {
            ...
            serde_json::to_vec(&record)
                .map_err(|_| "cannot encode managed transport status")?
        } else {
...
impl StatusReporter {
    ...
    pub async fn publish(&self, status: TransportStatus, terminal: bool) -> Result<(), String> {
        self.publish_snapshot(StatusSnapshot {
            status,
            terminal,
            connection_attempt: None,
            received_auth: None,
...
mod writer_tests {
    ...
    async fn v2_writer_persists_only_typed_exact_auth_evidence() {
        ...
        writer
            ...
            .unwrap();
        let record: TransportRecordV2 =
            serde_json::from_slice(&std::fs::read(fixture.0.join("status.json")).unwrap())
                .unwrap();
        assert_eq!(record.version, 2);
        assert_eq!(record.process_id, std::process::id());
        assert_eq!(record.received_auth.unwrap().classification, TransportAuthClassification::CommunityBanned);
        assert!(!String::from_utf8(std::fs::read(fixture.0.join("status.json")).unwrap())
            .unwrap()
