...
///
/// `auth_event_id` and `received_at_ms` are populated only for an ACK that was
/// correlated to the AUTH event sent by this attempt. Directly constructed
/// fixture errors can omit those fields without introducing relay text.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(super) struct AuthDeniedInfo {
...
impl AuthDeniedInfo {
    ...
    fn from_ack(auth_event_id: String, message: &str, received_at_ms: u64) -> Self {
...
impl HarnessRelay {
    ...
    pub async fn connect(
        ...
        let (ws, handshake_buffer) =
            transport_reconnect::retry_initial_connect_with_health_context(&mut health, |attempt| {
                do_connect(relay_url, keys, auth_tag.as_ref(), attempt)
            })
            .await?;
...
async fn do_connect(
    ...
    let ok =
        wait_for_auth_ok(&mut ws, &mut buffer, &auth_event_id, AUTH_TIMEOUT, &attempt).await?;
    if !ok.accepted {
        // Convert the relay-controlled denial at the exact-ACK boundary. The
        // raw message is not carried into the error or any later status path.
