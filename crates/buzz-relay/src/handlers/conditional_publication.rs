...
pub(crate) fn policy(
    ...
    if kind != KIND_REPO_WIKI_PAGE {
        if expected.is_some() {
            let d = singleton(event, "d")?.ok_or_else(|| invalid("missing d tag"))?;
            if d.is_empty()
                || d.len() > 64
                || d.starts_with('.')
                || d.contains("..")
        ...
        if !enabled && expected.is_some() {
            return Err(IngestError::Rejected(format!("unsupported: {EXTENSION}")));
        }
        return Ok(Some(revision.unwrap_or(PublicationPolicy::Legacy)));
    }
    let source_kind = singleton(event, "source-kind")?;
    // Validate singleton shape even though the full source-file projection is
    // owned by the Wiki envelope verifier.
    let _source_files = singleton(event, "wiki-source-files")?;
    let source_bound = SOURCE_BOUND_TAGS.iter().any(|name| {
        event
            .tags
            .iter()
    ...
    if !enabled && source_bound && !reserved {
        return Err(IngestError::Rejected(format!("unsupported: {EXTENSION}")));
    }
    if source_bound || reserved {
    ...
    if source_bound && !reserved && version != Some("1") {
        return Err(invalid("source-bound Wiki requires wiki-version 1"));
    }
    if version.is_some() {
        let source_kind = source_kind.ok_or_else(|| invalid("missing Wiki source kind"))?;
        if !matches!(source_kind, "git" | "folder") {
            return Err(invalid("invalid Wiki source kind"));
        }
    }
...
mod tests {
    ...
    fn conditional_policy_reserved_address_guard_survives_disabled_flag_and_omitted_tags() {
        ...
        assert!(matches!(
            policy(&replay, false).unwrap().unwrap().precondition(),
