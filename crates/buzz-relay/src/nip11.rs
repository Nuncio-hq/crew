...
// Keep production advertisement and its flag truth table on one seam.
    ...
    fn conditional_project_extension_requires_both_flags() {
        for conditional in [false, true] {
            for project in [false, true] {
                ...
                let extensions = info.supported_extensions.as_ref().unwrap();
                assert_eq!(
                    extensions
                        .iter()
                        .any(|v| v == crate::handlers::conditional_publication::EXTENSION),
                    conditional
                );
                assert_eq!(
                    extensions
                        .iter()
                        .any(|v| v == crate::handlers::conditional_publication::PROJECT_EXTENSION),
                    conditional && project
                );
                assert!(extensions.iter().any(|v| v == "nip-er"));
            }
        }
    }
