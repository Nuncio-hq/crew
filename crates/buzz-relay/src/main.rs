...
async fn main() -> anyhow::Result<()> {
    ...
    match state.db.verify_channel_roster_fence().await {
        ...
        Err(error) => {
            ...
            return Err(anyhow::anyhow!(
                "Channel roster fence is unsafe; apply or repair migration 0033 before starting this relay: {error}"
            ));
        }
    }
    ...
    // makes migration 0033 a code/schema compatibility gate before the new
