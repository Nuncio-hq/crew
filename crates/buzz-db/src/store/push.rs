...
mod postgres_tests {
    ...
    async fn batch_claim_is_single_community_and_setwise_ops_honor_the_fence() {
        ...
        // Use the same clock as the queue defaults: a remote database may be
        // slightly ahead of the process, which must not reorder this retry.
        let retry_at: DateTime<Utc> = sqlx::query_scalar("SELECT clock_timestamp()")
            .fetch_one(&pool)
            .await
            .expect("read queue clock");
