...
/// Verify that migration 0033's roster fence is active on the partitioned
...
/// Prove migration 0033's roster fence semantics through the live writer pool.
///
...
pub async fn lock_member_snapshot(
    ...
    // this key before INSERT; migration 0033 then takes the membership key in
...
mod postgres_tests {
    ...
    async fn large_roster_reconciliation_candidates_respect_snapshot_count_and_signer() {
        ...
        // Migration 0033's roster guard requires canonical four-field p tags
        ...
        // with UPDATE to model a stale snapshot that predates migration 0033's
    ...
    #[tokio::test]
    #[ignore = "requires Postgres"]
    async fn unmigrated_roster_fence_blocks_startup_until_0033_is_applied() {
        let admin = PgPool::connect(&admin_url().await)
            .await
            .expect("connect admin");
        let (pool, scratch_name) =
            create_scratch_db_through(&admin, "roster_fence_unmigrated", Some(32)).await;
        let db = Db::from_pool(pool.clone());

        let error = db
            .verify_channel_roster_fence()
            .await
            .expect_err("pre-0033 schema must block roster publishers");
        assert!(
        ...
        migration::run_migrations(&pool)
            .await
            .expect("apply migration 0033");
        db.verify_channel_roster_fence()
            .await
            .expect("0033 must open the startup gate");
