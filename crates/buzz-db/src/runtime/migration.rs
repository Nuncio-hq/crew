...
mod postgres_tests {
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        // NIP-PMA kind:30179 FTS exclusion (0034): same wrap-the-existing-
        ...
        // GIN index — see the 0034 header for the operational cost.)
        ...
        // fence as migration 0033. CI and isolated relay startup use schema.sql
        ...
        let relay_operators = migrations[35].sql.as_str();
        assert!(
        ...
        let relay_admin_actions = migrations[36].sql.as_str();
        assert!(
        ...
        let action_lease = migrations[37].sql.as_str();
        assert!(
        ...
        let operator_audit = migrations[39].sql.as_str();
        assert!(
        ...
        let identity_foundation = migrations[41].sql.as_str();
        ...
        let authorization_foundation = migrations[42].sql.as_str();
        ...
        let dogfood_profile = migrations[43].sql.as_str();
        ...
        // Drop the Phase-A NIP-FI relay-side authority ledger (0042 + 0043).
        ...
        // to its pre-0042 body so the deletion catalog no longer includes the
        // removed relations.
        assert_eq!(migrations[44].version, 45);
        let ledger_removal = migrations[44].sql.as_str();
        ...
        // schema.sql exclusion list must match the restored (pre-0042) body.
        assert!(
    ...
    fn push_match_trigger_is_narrowed_to_message_kinds_additively() {
        ...
        let sql = migrations[40].sql.as_str();
    ...
    /// migrations (0036/0037) once drifted for exactly this reason.
    ///
    ...
    async fn populated_upgrade_preserves_search_policy_except_for_private_kinds() {
        ...
        // tokenized kind:30179 ciphertext — the gap 0034 closes.
        MIGRATOR
            .run_to(33, &pool)
            .await
            .expect("apply migrations through 33");
        crate::store::channel_members::verify_channel_roster_fence_behavior(&pool)
            .await
            .expect("Crew roster fence remains active before upgrade");
        let pre_0034: Vec<(i32, Option<bool>)> = sqlx::query_as(
            "SELECT kind, search_tsv @@ plainto_tsquery('simple', 'needle') \
             FROM events ORDER BY kind",
        )
        .fetch_all(&pool)
        .await
        .expect("read pre-0034 search behavior");
        assert_eq!(
            pre_0034,
            vec![
                (1, Some(true)),
        ...
        crate::store::channel_members::verify_channel_roster_fence_behavior(&pool)
            .await
            .expect("Crew roster fence remains active after upgrade");
    ...
    async fn run_migrations_applies_consolidated_initial_schema_on_fresh_database() {
        ...
        assert!(
            search_expression.contains("ARRAY[0, 9, 40002, 45001, 45003, 30023, 30623]"),
    ...
    /// Verify migration 0045 applies cleanly against a DB that has rows in
    /// the NIP-FI 0042+0043 tables. The immutability guards (no_delete,
    /// no_truncate) are enforced via triggers; DROP TABLE bypasses them and
    /// must succeed even when rows are present.
    #[tokio::test]
    #[ignore = "requires Postgres"]
    async fn migration_0045_drops_populated_nip_fi_ledger_cleanly() {
        let pool = connect_test_pool().await;
        reset_public_schema(&pool).await;
        MIGRATOR
            .run_to(43, &pool)
        ...
        // Seed a community and minimal rows in a selection of 0042+0043 tables.
        let community_id = uuid::Uuid::new_v4();
        sqlx::query("INSERT INTO communities (id, host) VALUES ($1, $2)")
            .bind(community_id)
        ...
        // Seed a receipt (used as FK anchor for several 0043 tables).
        ...
        // Seed an invalidation domain (0043 table with no FK to receipts).
        sqlx::query(
        ...
        // Apply migration 0044 (dogfood profile) and 0045 (ledger removal).
        MIGRATOR
            .run_to(45, &pool)
            .await
            .expect("migration 0045 must apply cleanly against a populated NIP-FI DB");
