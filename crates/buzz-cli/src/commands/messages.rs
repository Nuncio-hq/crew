...
/// Accept canonical links and the existing `--event buzz://message` form.
fn resolve_thread_input(
    channel: Option<String>,
    event: Option<String>,
    link: Option<String>,
) -> Result<(String, String, Option<String>), CliError> {
    let legacy_link = event
        .as_deref()
        .is_some_and(|value| value.starts_with("buzz://"));
    if let Some(link) = link
        .as_deref()
        .or_else(|| legacy_link.then_some(event.as_deref()).flatten())
    {
        let parsed = crate::links::parse_message_link(link)?;
        if let Some(channel) = channel {
            let supplied = Uuid::parse_str(&channel)
                .map_err(|_| CliError::Usage("invalid channel UUID".into()))?;
            if supplied.to_string() != parsed.channel_id {
                return Err(CliError::Usage(
                    "--channel does not match the link's channel".into(),
                ));
            }
        }
        return Ok((parsed.channel_id, parsed.message_id, parsed.thread_root_id));
    }
    match (channel, event) {
        (Some(channel), Some(event)) => Ok((channel, event, None)),
        _ => Err(CliError::Usage(
            "messages thread requires --link, an --event Buzz link, or both --channel and --event"
                .into(),
        )),
    }
}
...
pub async fn dispatch(
    ...
    match cmd {
        ...
        MessagesCmd::Thread {
            ...
            let (channel, event, expected_root) = resolve_thread_input(channel, event, link)?;
            cmd_get_thread(
                client,
                &channel,
                &event,
...
mod tests {
    #[test]
    fn thread_input_keeps_legacy_event_links_and_checks_explicit_channel() {
        ...
        assert_eq!(
            super::resolve_thread_input(None, Some(link.clone()), None).unwrap(),
            (channel.into(), event.clone(), None),
        );
        assert!(
            super::resolve_thread_input(Some(channel.into()), Some(link.clone()), None).is_ok()
        );
        assert!(
            super::resolve_thread_input(Some(Uuid::new_v4().to_string()), Some(link), None)
                .is_err()
        );
        assert!(super::resolve_thread_input(None, Some(event), None).is_err());
    }

    #[test]
    fn blank_send_requires_an_attachment() {
        assert!(super::reject_empty_send_content(" \n\t", &[]).is_err());
        assert!(super::reject_empty_send_content("", &["image.png".into()]).is_ok());
        assert!(super::reject_empty_send_content("hello", &[]).is_ok());
    }
