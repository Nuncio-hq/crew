...
CREATE TABLE contact_routes (
    ...
    canvas_id BYTEA CHECK (canvas_id IS NULL OR octet_length(canvas_id) = 32)
);
    ...
    IF TG_OP = 'INSERT' THEN
        IF NEW.kind = 9 THEN
            IF NEW.contact_class IS NULL THEN
                NEW.contact_class := 0;
            ELSIF NEW.contact_class <> 0
               AND current_setting('buzz.contact_decision_v1', true) <> 'on'
            THEN
...
-- Ordinary hard deletion of a kind-9 original or contact decision proof is
-- forbidden. The existing
    ...
    IF TG_OP = 'UPDATE' THEN
        IF OLD.contact_class IS DISTINCT FROM NEW.contact_class THEN
            RAISE EXCEPTION 'contact classification is immutable'
                USING ERRCODE = 'check_violation';
        END IF;
        IF (OLD.kind IN (9, 46044) OR NEW.kind IN (9, 46044))
        ...
        RETURN NEW;
    END IF;
    IF OLD.kind NOT IN (9, 46044) THEN
        RETURN OLD;
    END IF;
...
-- Channel roster snapshot fence (keep in sync with migrations/0033).
