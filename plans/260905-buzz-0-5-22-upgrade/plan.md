...
PR #342 open, previous-head Gate/manual checks passed, focused browser repairs verified; new-head checks pending. No merge, release or hosted acceptance claimed.
...
- [x] Run local `just ci`: checks/builds passed; desktop 6991 passed + one skip,
  ...
  browser reconciliation: 7012 passed + one existing skip, zero failures (7013 total).
...
- [x] Open PR #342; Gate job and manual sync passed on `7b58b8d2`. The encompassing
  CI workflow was cancelled after browser failures.
...
- [ ] Push repair revision and rerun required remote gates against that exact head.
...
Unresolved: remote gates on the repair revision.
