...
| 06 — capability view | Moved to R3, gated on S-B and the still-open Q2 ownership question. #149 already ships #118's narrower `{ modelSource, personaDoc, layer3 }` descriptor; R3 covers the expanded tools/skills/MCP surface without creating a second card. | `desktop/src/shared/api/runtimeCapabilities.ts:1-14`; `desktop/src/shared/api/fromRawAcpRuntimeCatalog.ts:44-83` on PR #149 / branch `agents/hermes-profile-editing`; `crates/buzz-acp/src/config.rs:735-743` |
