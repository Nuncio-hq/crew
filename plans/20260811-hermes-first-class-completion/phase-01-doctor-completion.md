...
The evaluator already resolves and probes the binary, checks the profile
directory, parses `config.yaml`, and exposes `Ready`, `Missing`,
`BrokenConfig`, `BinaryMissing`, and `AuthUnknown`
(`/home/ubuntu/hermes-104-evidence.md:413-430`). This phase adds only the
missing compatibility/dependency/update diagnostics; it does not add an auth
probe or auth badge.
...
| ID | Topic | Choice |
| --- | --- | --- |
| D1 | State model | Preserve #134's `AuthUnknown`; add `Incompatible` only after S-A PASS |
| D2 | Profile check | Keep the existing bound-directory/config checks and their blocking behavior |
| D3 | ACP probe | Invoke `hermes acp --check` read-only with bounded timeout and sanitized output |
| D4 | MCP probe | Check the resolved `buzz-dev-mcp` executable, not a human-readable reply |
| D5 | Refresh | Add an explicit Hermes Doctor re-check action wired through existing IPC/query invalidation |
| D6 | Repair | Show a copyable command and official documentation link per actionable state |
| D7 | Updates | Add `hermes update --check` only after S-C PASS; never run update |
| D8 | Auth | No auth badge; no text scraping; `AuthUnknown` remains honest |
...
(`/home/ubuntu/hermes-104-evidence.md:494-503`).
...
| --- | --- | --- | --- |
| D-01 | Resolved binary is absent | `BinaryMissing`/actionable state with no spawn | Silent `Ready` |
| D-02 | Version is below S-A minimum | `Incompatible` with observed/required versions | Starting the profile |
| D-03 | Version output is malformed | `NotVerified`/unknown diagnostic | Guessing compatibility |
| D-04 | `hermes acp --check` fails | Dependency failure is surfaced with bounded, sanitized detail | Treating exit 0 from another command as proof |
| D-05 | `buzz-dev-mcp` is unavailable | Actionable MCP repair state and command | Reporting an auth failure as authentication truth |
| D-06 | Bound profile directory/config is absent or invalid | Existing #134 missing/broken state remains blocking | Binding or starting anyway |
| D-07 | Re-check after external repair | IPC/query refreshes state without app restart | Stale card after successful re-check |
| D-08 | Actionable state displayed | Copyable repair command plus official link | Uncopyable or invented URL |
| D-09 | `hermes update --check` says update available | Read-only update fact is shown | Installing, rewriting, or mutating profile state |
| D-10 | Diagnostic contains paths, tokens, or secrets | Only allowlisted fields/classes cross the presenter boundary | Raw stdout/stderr in UI, logs, or relay |
| D-11 | Auth cannot be probed truthfully | State is `AuthUnknown`; no auth badge | Inferring auth from version/check text |
...
These paths exist on #134; the evidence report records the evaluator and
generic refresh plumbing (`/home/ubuntu/hermes-104-evidence.md:413-436`).

## Implementation after approval

After S-A/S-C are conclusive and D-01…D-11 are RED, implement the smallest
...
- [ ] D-01…D-11 are RED first, then GREEN with focused tests.
