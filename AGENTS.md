...
just bootstrap           # create local environment and stable relay identity
...
generated hook checks `LEFTHOOK_BIN`, then `PATH`, then installation fallbacks.
`lefthook.yml` does not configure `rc:`, so `bin/.lefthookrc` is not sourced;
activate Hermit before invoking hooks to select the pinned lane toolchains.
After Lefthook selects files, `scripts/hook-lane-wrapper.sh` clears canonical
repository-local Git variables (including invoking-command config overrides)
only in each check/test lane and verifies its physical cwd is the repo root.
This keeps nested Git fixtures from writing the caller's shared worktree metadata
while preserving the parent hook's diff/staging context. Wrapper/config edits
run the fast isolation contract; existing native lane globs remain unchanged.
...
The existing `--event buzz://message?...` form remains supported; an explicit
`--channel` must match its channel.
...
Crew keeps Cmd +/- zoom text-only: `useWebviewZoomShortcuts.ts` calls
`applyTextZoomFactor` in `fontSizePreference.ts`, which composes the keyboard
factor with the font-size preference in the virtual typography scale. The real
root font-size and native webview coordinates stay fixed, so spacing and panels
do not resize. **Use named text tokens so readable text follows this scale.**
...
- ✅ Conversation text uses the named `text-message` token. Its
  **Smaller / Default / Larger contract is 13 / 14 / 15px** before keyboard
  ...
  fixed chat baseline or hardcoding preference-specific values in
  components.
...
- **Keep widgets small and composable.** One public widget per file; push
  ...
  **1000 lines/file**, enforced with the other surface-specific limits by the
  ...
  Upstream-heavy files may use exact recorded baselines; Crew additions still
  require extraction under D-022. Do not raise the surface limit during a sync.
...
The dev relay runs with `BUZZ_REQUIRE_AUTH_TOKEN=false` and the stable relay
identity generated in `.env` by `just bootstrap`. No public signing-key fallback
is available; load the generated environment before starting the relay.
