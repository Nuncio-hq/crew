...
export const sections = [
  ...
  },
  {
    ...
    open: "Prototype dùng bài mẫu và câu trả lời chuẩn bị trước; không có LLM/indexing thật. Company handbook không bị xóa; vị trí truy cập nó trong layout mới còn cần chốt. Private-history ACL/persistence và dispatch thật chưa được nối.",
    seam: "Tái dùng crew-wiki, WikiPageView/WikiTocSidebar/WikiSourcePanel, wikiEvents và wikiHooks. Không tạo knowledge database song song. wikiAsk hiện là placeholder: cần QA với source snapshot, citations, cancel/retry và handoff vào channel/thread hiện hữu.",
  },
  {
    id: "agents",
    number: "03",
  ...
  },
  {
    ...
    actions:
      "Tên Project hoặc breadcrumb mở trang Project; mũi tên chỉ bung/thu channels. Workspace Channels bung/thu danh sách trực tiếp, không có trang trung gian; Browse channels nằm trong menu workspace kể cả khi mọi channel chung đã liên kết Project. Add project nhập tên, folder tùy chọn, main channel mới/có sẵn. Add channel, Link folder, Manage và Unlink dùng dữ liệu mẫu trong bộ nhớ.",
    boundary:
      "Founder yêu cầu thiết kế để review ngày 2026-09-09; chi tiết UI chưa được chấp nhận để triển khai production. Folder picker, Git detection và việc tạo/liên kết chỉ mô phỏng. Không gọi native API, sửa Git, folder, relay hay active agent.",
    open: "Cần review UI và thống nhất luồng folder legacy Repository với explicit Project. Đường dẫn chính xác, truy cập theo host, retry publish/readback và channel membership cần nguồn thật. Folder không truy cập được phải có trạng thái riêng.",
  ...
  },
  {
    ...
    shows:
      "Tabs Files, Browser, PRs, Actions, Terminal, Context trong bản đề xuất. Plan thuộc Context/Activity, không chiếm thêm cột mặc định.",
    actions:
...
export const feasibility = [
  {
    ...
    source:
      "AppShell / AppSidebar, ChannelPane / MessageThreadPanel; NIP-MP kind 30621 nhóm repositories 30617 với buzz-channel canonical.",
    remaining:
      "Đưa layout D-078 vào app hiện có; xác định quan hệ một project với nhiều department channels. Sửa guardrail check-channel-first-ia đang cấm Projects; giữ cấm Workbench picker. Không tạo registry React song song.",
    limit:
      "D-056 còn yêu cầu plan rail luôn hiện, prototype dùng tab; cần ghi rõ quyết định thay thế khi chuyển UI. #344 đang tả Workbench route đã redirect.",
  },
  {
  ...
  },
  {
    ...
    remaining:
      "Chuyển presenter vào layout mới, giữ owner scope, agent/conversation/session và empty/retired snapshot semantics. Re-measure latency trong #344.",
    limit:
      "Queue hiện pack nhiều events cùng channel vào một frame mỗi tick; mô tả một event/tick trong #344 đã cũ. Chưa đo latency live lần này.",
  },
  {
    ...
    remaining:
      "Tái dùng lifecycle đầy đủ trong UI mới; giữ channel-join acknowledgement, remote shutdown/orphan guard, identity history và cleanup retry.",
    limit:
  ...
  },
  {
    ...
    limit:
      "set_canvas không có expected revision. Canvas publish và announcement là hai writes: có thể partial success. Tắt require_mention không đủ để chặn bot loops hoặc tin đang gọi người khác.",
  },
  {
...
export const backlogAudit = [
  {
    title: "#344 — rewrite thành CompanyOS workspace integration",
    url: "https://github.com/Nuncio-hq/crew/issues/344",
    action:
      "Giữ issue này làm nơi tích hợp layout/focus: mapping Projects, migration guardrails, channel/thread + tools, activity/plans và CRUD sẵn có. Chia thành PR theo lát cắt có thể test; không tạo issue cho từng nút. Giữ latency là mục đo/khắc phục với baseline mới, không chép lại chẩn đoán cũ.",
  },
  {
    title: "Mới 1 — Channel roles editor + contact point",
    action:
  ...
  {
    title: "Mới 2 — Runtime-backed thread recap",
    action:
  ...
  {
    title: "Mới 3 — D-077 staging và coding-delivery acceptance",
    action:
  ...
  },
  {
    ...
    action:
      "#345 là thảo luận độ sâu fork delta. #347 là PR docs đang mở; local HEAD đang đúng head của PR này, với blueprint/docs mới chưa commit. Review/reconcile docs trước khi tạo PR implementation từ main.",
  },
  {
