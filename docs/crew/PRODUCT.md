...
| One daily self-hosted relay | The company's live relay is on dev-server over Tailscale; isolated staging on that same host is agreed separately (D-077) |
...
- Sidebar top: Inbox, Agents, Workflows (2026-09-09 refinement).
- Sidebar middle: Projects, each expanding to Wiki and its channels. Shared
  channels remain accessible through Browse channels in the workspace menu.
