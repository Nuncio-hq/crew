...
Buzz relay (signed events; shared state)
  ...
  |
  v
buzz-acp -> provider ACP adapter -> employee runtime (primary: Hermes)
                                  |
                                  v
                         profile memory, skills, and task tools
```

The relay is the shared coordination log. It is not the source-code store.

**Daily deployment (verified 2026-09-08):** the founder's live relay is
self-hosted on `oscar-dev-server`, reachable over Tailscale at
`ws://100.86.143.13:3000`. `buzz-relay` runs as a host process; PostgreSQL,
Redis and MinIO run in Docker on the same server. The earlier third-party
`lilgroup.communities.buzz.xyz` community is deprecated, although imported
local agent records can still retain that URL. Verify effective destinations
rather than assuming every stored agent already targets the daily relay.

**Agreed test deployment (D-077; not yet provisioned):** staging also runs
on dev-server, with its own relay and backing services/storage. The local
Crew build and test agents connect over Tailscale. Data flows one way from
a daily-system snapshot to staging; the two environments never share writable
data. The canonical snapshot, configuration isolation, and reset policy is
in [`TESTING.md`](TESTING.md#test-environments-and-real-data-staging-d-077).
...
Role labels resolve from owner-signed channel canvas assignments (`crew_role.rs`, D-043/D-044); thread owner and participants do not redefine them. The prototype's sample channel roster is not an authoritative registry or a capability source. Remaining project grouping, queue/acceptance automation and external service integration are still proposals or unverified integration work.
