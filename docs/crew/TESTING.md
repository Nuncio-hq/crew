...
The source seam uses chronological cursor pagination, newest-window selection,
auxiliary-event exclusion, and the `source_limit` failure when the bounded
relay scan cannot prove EOF; an overflowed source is never reported as a
current recap. The focused source tests bind the production builder for
newest-window selection and auxiliary projection.
...
guard, source scan limit, or EPERM reap condition must fail the corresponding
production-seam regression. Synthetic
