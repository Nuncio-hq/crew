...
Schema v4 and permanent-dependency Regenerate are accepted designs whose runtime
...
communities, payloads, revisions and claims. Reopening v4 must be idempotent;
...
directory, verify a v4-capable build and reopen the same journal. Set the paths
...
: "${CREW_VERIFIED_V4_APP:?Set the verified v4-capable app bundle}"
umask 077
CREW_RECOVERY_DB="$CREW_APP_DATA/owner-operations/recovery.db"
CREW_RECOVERY_COPY="$(mktemp -d "${TMPDIR:-/tmp}/crew-recovery-v4.XXXXXX")"
...
open "$CREW_VERIFIED_V4_APP"
```

Run those commands only after app shutdown, with `CREW_APP_DATA` and
`CREW_VERIFIED_V4_APP` already set to verified absolute paths. Preserve any
SQLite sidecar in the directory copy. Expect schema `4` and integrity `ok`;
