...
Crew deliberately keeps presence informational for Start and Restart. A locally
stopped record remains startable when exact-key presence is Online or Away;
presence must not hide or disable its existing local/pair lifecycle action.
Running local/pair controls keep Stop/Restart routing even when presence is
Offline. One agent may participate in many independent thread sessions; relay
presence does not identify or exclusively own a local process. The existing
native ownership and runtime rules still authorize each lifecycle mutation.
Runtime details remain accessible through the card/profile.
...
- `desktop/tests/e2e/agent-availability.spec.ts`: deployed/offline profile and
  card, stopped/Online and Away presence with available keyboard/click Start
  actions, offline startup, running-but-Offline,
